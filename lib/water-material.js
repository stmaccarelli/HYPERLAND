...
THREE.ShaderLib['water_hy'] = {
  ...
  fragmentShader: [
    ...
    '{',
    '  vec2 uv0 = uv / (103.0 * noiseScale) - vec2(time / 17.0, time / 29.0) + vec2(0.0,time*0.1);',
    '  vec2 uv1 = uv / (107.0 * noiseScale) - vec2(time / -19.0, time / 31.0) + vec2(0.0,time*0.1);',
    '  vec2 uv2 = uv / (vec2(8907.0, 9803.0) * noiseScale) - vec2(time / 101.0, time /   97.0) + vec2(0.0,time*0.02);',
    '  vec2 uv3 = uv / (vec2(1091.0, 1027.0) * noiseScale) - vec2(time / 109.0, time / -113.0) + vec2(0.0,time*0.02);',
    ...
    ' vec3 mirrorDistord = mirrorCoord.xyz + vec3(distortion.x, distortion.y-4.0, 1.0);',
    ...
    '  vec3 scatter = max(0.0, dot(distordNormal, eyeDirection)) * vec3(0.1);',
    'vec3 mirrorFinal; vec3 specularFinal;',
    // blur mirror image
    'if(blur){',
    ...
    '   blurSample += texture2DProj(mirrorSampler, mirrorDistord+vec3( 0.0  ,-1.1*i,0.0)).xyz;',
    '   blurSample += texture2DProj(mirrorSampler, mirrorDistord+vec3( 1.0*i,-1.1*i,0.0)).xyz;',
    ...
    '   blurSample += texture2DProj(mirrorSampler, mirrorDistord+vec3( 1.0*i, 1.1*i,0.0)).xyz;',
    ' };',
    ' blurSample = (blurSample/8.0/(8.0/4.0));',

    //compute final mirror and specular
    ' mirrorFinal=(reflectance-scatter*2.0) * blurSample - specularLight*scatter;',
    ' specularFinal = specularLight*reflectionSample*vec3(blurSample.r*blurSample.g)*1.9;',
    '}',
    'else{',
    ' mirrorFinal=(reflectance-scatter*2.0) * reflectionSample - specularLight*scatter;',
    ' specularFinal = specularLight*reflectionSample*vec3(reflectionSample*reflectionSample)*1.9;',
   '}',

    // Compute final pixel color
    'gl_FragColor = vec4( color * scatter * 5.0 + mirrorFinal*0.5 + mirrorFinal*color*0.5 + specularFinal, alpha-length(scatter)*0.7 );',//' , alpha + (reflectance*(1.0-alpha)) );',//alpha );',
...
THREE.Water = function (renderer, camera, scene, options) {
  ...
  this.color = new THREE.Color(optionalParameter(options.color, 0x003344));
  this.waterReflectivity = optionalParameter(options.waterReflectivity, 0.75);
  this.blur = optionalParameter(options.blur, true);

  this.eye = optionalParameter(options.eye, new THREE.Vector3(0, 0, 0));
  this.distortionScale = optionalParameter(options.distortionScale, 50.0);
